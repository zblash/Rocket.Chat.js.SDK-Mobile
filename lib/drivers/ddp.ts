...
export class Socket extends EventEmitter {
  ...
  disconnectedBcsOfTimeout: boolean;
  loggedInUser: any;
  ...
  open = (ms: number = this.config.reopen) => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        connection = new WebSocket(this.host, null, {
      ...
      if (this.loggedInUser && this.isDisconnected) {
        ...
        this.login(this.loggedInUser)
          .then(() => {
            this.logger.debug("[ddp] Reconnected because of timeout");
          })
          .catch((err) => {
            this.logger.error(
              `[ddp] Unable to reconnect because of timeout: ${err.message}`
            );
          });
      }
  ...
  onOpen = async (callback: Function) => {
    ...
    this.ping().catch((err) =>
      this.logger.error(`[ddp] Unable to ping server: ${err.message}`)
    );
    this.emit("open");
  ...
  send = async (obj: any): Promise<any> => {
    return new Promise(async (resolve, reject) => {
      ...
      if (!this.connected)
        await new Promise((resolve) => this.on("open", resolve));
      ...
      if (/^sub$/.test(obj.msg)) {
        ...
        this.subscriptions[id] = {
          ...
          name,
          params,
          unsubscribe: this.unsubscribe.bind(this, id),
        };
      }
  ...
  call = async (method: string, ...params: any[]) => {
    const response = await this.send({ msg: "method", method, params }).catch(
      (err) => {
  ...
  unsubscribeAll = () => {
    ...
    return Promise.all(unsubAll).then(() => (this.subscriptions = {}));
  };
}
